...
if (!in_array($fundType, ['cash', 'bank', 'company', 'custom'], true)) $fundType = 'cash';
if ($fundType === 'bank' && !$bankId && $bankAccounts) $bankId = (int)$bankAccounts[0]['id'];
if ($fundType === 'custom' && !$fundId && $customFunds) $fundId = (int)$customFunds[0]['id'];

$fromDate = $_GET['from'] ?? date('Y-m-01');
$toDate = $_GET['to'] ?? date('Y-m-d');

// So du dau ky (truoc fromDate)
function fund_balance_before(PDO $pdo, string $type, int $bankId, int $fundId, string $beforeDate): float
{
    ...
    if ($type === 'custom') { $sql .= ' AND fund_id = ?'; $params[] = $fundId; }
...
if ($fundType === 'custom') { $sql .= ' AND fl.fund_id = ?'; $params[] = $fundId; }
$sql .= ' ORDER BY fl.tx_date ASC, fl.id ASC';
$stmt = $pdo->prepare($sql);
$stmt->execute($params);
$entries = $stmt->fetchAll();

$balance = fund_balance_before($pdo, $fundType, $bankId, $fundId, $fromDate);
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
  ...
  <div class="d-flex gap-2">
    <a href="<?= url('/funds/manage.php') ?>" class="btn btn-outline-secondary"><i class="bi bi-journal-plus"></i> Thêm sổ quỹ mới</a>
    <a href="<?= url('/funds/form.php?type=' . $fundType . '&bank_id=' . $bankId . '&fund_id=' . $fundId) ?>" class="btn btn-success"><i class="bi bi-plus-lg"></i> Thêm giao dịch</a>
  </div>
</div>
...
<ul class="nav nav-pills mb-3 flex-wrap">
  ...
  <?php foreach ($customFunds as $cf): ?>
    <li class="nav-item">
      <a class="nav-link <?= $fundType === 'custom' && $fundId === (int)$cf['id'] ? 'active' : '' ?>" href="<?= url('/funds/index.php?type=custom&fund_id=' . $cf['id']) ?>"><i class="bi bi-wallet2"></i> <?= e($cf['name']) ?></a>
    </li>
  <?php endforeach; ?>
  <li class="nav-item">
    <a class="nav-link text-muted" href="<?= url('/funds/manage.php') ?>" title="Thêm sổ quỹ mới"><i class="bi bi-plus-circle"></i></a>
  </li>
</ul>
